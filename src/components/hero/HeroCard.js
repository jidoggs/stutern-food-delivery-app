...
function HeroCard({ image, type, discount, name, color }) {
  return (
    <div style={{ backgroundImage: `url(${image})` }} className="hero__card">
      <Link
        to={"/deals"}
        className={`hero__card--wrapper ${color && "pinkyBg"}`}
      >
        <span
        ...
        <div className="hero__card-text">
          ...
          <h2
            className={`title__main mb2-25   ${
              color ? "pinkyTxt" : "regHeadlineTxt"
            } `}
          >{`${discount}% OFF`}</h2>
